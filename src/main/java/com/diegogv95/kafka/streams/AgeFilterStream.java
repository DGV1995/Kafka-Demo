...
@NoArgsConstructor
public class AgeFilterStream {

    public static void main(String args[]) {
        new AgeFilterStream().run();
    }
    ...
    public Topology createTopology() {
        ...
        KStream<String, String>[] filteredUsers = inputStream.branch(
                (key, value) -> gson.fromJson(value, User.class).getAge() >= 50,
                (key, value) -> gson.fromJson(value, User.class).getAge() < 50
        );

        filteredUsers[0].to(AppProperties.AGE_GREATHER_THAN_50_TOPIC, Produced.with(Serdes.String(), Serdes.String()));
        filteredUsers[1].to(AppProperties.AGE_LESS_THAN_50_TOPIC, Produced.with(Serdes.String(), Serdes.String()));
